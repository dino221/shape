...
public class ListNetworkInterfaces {
    ...
    public static void main (String[] args)
        throws SocketException {
            ...
            while(nics.hasMoreElememts()) {
                NetworkInterface nic = nics.nextElement();
                showInfo(nic);
            }
}

private static void showInfo (NetworkInterface nic)
    throws SocketException {
        System.out.printf("Name: %s%n", nic.getName());
        System.out.printf("Index: %d%n", nic.getIndex());
        System.out.printf("Description: %s%n",
        nic.getDisplayName());
        System.out.printf("MTU: %d%n", nic.getMIU());   
        System.out.printf("Loopback: %b%n", nic.isLoopback());
        System.out.printf("PointToPoint: %b%n",
        nic.isPointToPoint());
        System.out.printf("Up: %b%n", nic.isUp());
        System.out.printf("Supports multicast: %b%n",
        nic.supportMulticast());
        System.out.printf("%b%n", nic.isVirtual());
        System.out.printf("Hardware address: %s%n",
            Util.toHex(nic.getHardwareAddress()) );
        Enumeration<InetAddress> addrs = nic.getInetAddresses();
        while(addrs.hasMoreElements()) {
            InetAddress addr = addrs.net.Elemet();
            System.out.printf(" Internet address: %s%n",
                Util.addrToString(addr));
        }
        System.out.println(" ----");
